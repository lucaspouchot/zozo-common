...
export function ensureString(o: any): string {
...
export function ensureNumber<T>(n: any, onNaN:T): number|T {
    n = Number(n);
    if (isNaN(n)) {
        return onNaN;
    }
...
// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types, @typescript-eslint/no-explicit-any
export function ensureInt<T>(n: any, base = 10, onNaN:T): number|T {
...
// eslint-disable-next-line @typescript-eslint/explicit-module-boundary-types, @typescript-eslint/no-explicit-any
export function ensureBool(b: any, def = false): boolean {
...
export function capitalize(str?: string): string {
    ...
    str = str.replace(/[\u00A0\u200B\u202F\u2060\u2420\u2422\u2423\u3000\uFEFF]/g, ' ');
    const prefix = ['l', 'd', 'c'];
    const lower = ['le', 'du', 'de', 'a', 'et', 'la'];
    const upper = ['ffkmda'];
    const newStr = str.split(' ').filter(item => item).map(itemSpace => {
        if (itemSpace === '-') {
            return itemSpace;
        }
        return itemSpace.split('-').filter(item => item).map(item => {
            let pre = '';
            if (item.length >= 1 && prefix.indexOf(item[0].toLowerCase()) !== -1 && item[1] === '\'') {
                pre = item.substr(0,2).toLowerCase();
                item = item.substr(2);
            }
            if (lower.indexOf(item.toLowerCase()) !== -1) {
                return `${pre}${item.toLowerCase()}`;
            }
            if (upper.indexOf(item.toLowerCase()) !== -1 || (item.length > 2 && (item.match(/\./g) || []).length === (item.length-1)/2)) {
                return `${pre}${item.toUpperCase()}`;
            }
            return `${pre}${item[0].toUpperCase()}${item.slice(1)?.toLowerCase()||''}`;
        }).join('-');
    }).join(' ');
...
for (let index=0, max=defaultDiacriticsRemovalMap.length; index<max; index++) {
    const letters = defaultDiacriticsRemovalMap[index].letters;
    for (let indexLetter=0, maxLetter=letters.length; indexLetter<maxLetter; indexLetter++) {
        diacriticsMap[letters[indexLetter]] = defaultDiacriticsRemovalMap[index].base;
    }
}
...
export function normalisePropertyExpression(str) {
...
export function getObjectProperty(obj: any, expr: string) {
    let result = null;
    let propParts = null;
    try {
